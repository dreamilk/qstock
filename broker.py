...
print(d[(d["代码"]==symbols[0]) | (d["代码"]==symbols[1]) | (d["代码"]==symbols[2])])
...
def buy_with_stop_loss(ctx: ExecContext):
    ...
    if not pos:
        ...
        df = akshare.query(symbols=symbols, start_date=ctx.date, end_date=ctx.date)
        print(df)
...
strategy.add_execution(fn=buy_with_stop_loss, symbols=symbols)
